...
Anywhere you see `[ ]` is a real placeholder waiting on a fact from Sharjeel/David. **Don't invent a number, quote, or price to fill it in.** Build the slot, flag it visibly (e.g. a clearly-labeled placeholder card, not text that could pass as real), and log it under "Waiting on client" in `HANDOVER.md`.
...
## 2. Trust bar
Row of 3–4 quick facts directly under the hero, Scoreboard Amber numbers + Chalk White labels:
- `[ ]` years combined coaching experience — *(get real number from David)*
- `[ ]` athletes trained — *(get real number from David)*
...
2. **Single Lesson** — `[ ] price` — "One-on-one session, any skill focus." CTA: Book a Lesson
3. **4-Lesson Package** *(featured — Clay Red top border)* — `[ ] price` — "Buy 4, get 1 free. Best for consistent progress." CTA: Book My Free Evaluation
...
`[ ]` — confirm real pricing with David before launch. **Do not ship placeholder prices that look like real numbers.**
