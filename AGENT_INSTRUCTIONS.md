...
## v4 update — finishing pass, not a new direction
The v3 rebuild was reviewed and is a real improvement: photography landed, single accent color held, hero composition works, the Lineup section is fixed. It's not done yet for four specific, fixable reasons — read `DESIGN_SYSTEM.md`'s new "v4" section in full, it covers: inconsistent section spacing, the HitTrax stat row having no visual anchor, the dark testimonial section not being full-bleed, and inconsistent photo treatment. Fix exactly those four things plus the performance issue below — don't redesign sections that are already working (hero, Lineup, single-accent-color enforcement all stay as-is).

## Performance is a launch blocker, not polish
The last build took ~2 seconds for images to paint in. Read `DESIGN_SYSTEM.md`'s "Performance" section and implement it literally: Next.js `<Image>` everywhere (never raw `<img>`), `priority` on the hero photo only, WebP exports, explicit dimensions to prevent layout shift. Verify with actual Network tab timing before marking this done — "looks fine" is not the same as "loads fast," and this has been a stated requirement since the very first version of this file.

## Placeholder numbers — updated rule, read carefully
This changed from earlier sessions: `CONTENT_SPEC.md` now has realistic placeholder numbers filled in for the trust bar and pricing (45+ years experience, 800+ athletes trained, $65/$240 pricing) — use these as given, rendered with a visible `PLACEHOLDER` markup tag (data attribute or HTML comment) next to each one. This is different from testimonials, which still must NOT be invented — leave Section 7 as an empty, clearly-labeled pending slot. Don't apply the "never invent" rule to the numeric placeholders anymore; that rule now only covers testimonials.
